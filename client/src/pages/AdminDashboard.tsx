...
import { Lock, CheckCircle2, XCircle, ArrowLeft, LogOut, MapPin, Globe, Monitor, Clock, Laptop, Smartphone, X, Eye } from "lucide-react";
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-[440px] max-h-[90vh] overflow-y-auto bg-[#1e293b] border border-[#334155] rounded-[20px] shadow-[0_25px_80px_rgba(0,0,0,0.7)]" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 pt-3 flex gap-2">
          ...
          <button onClick={onClose} className="h-[44px] px-6 rounded-[12px] bg-white/5 hover:bg-white/10 border border-white/10 text-white font-mono text-[13px] font-bold">Zamknij</button>
        </div>
      </div>
    </div>
  );
...
function BlockedCard({ id, attempts, onDetails }: { id: string, attempts: any[], onDetails: (a:any)=>void }) {
  ...
  const details = attempts.find((a: any) => a.ipAddress === id || a.fingerprint === id) || attempts[0] || null;
  const info = details;

  return (
    <div className="bg-[#0f172a] border border-orange-500/30 rounded-xl p-4 hover:border-orange-500/60 transition-all flex flex-col group/card">
      <div className="flex justify-between items-start mb-3">
        <span className={`flex items-center gap-1.5 text-[10px] font-mono px-2.5 py-1 rounded-full font-bold border ${isIp ? 'bg-blue-500/10 text-blue-300 border-blue-500/30' : 'bg-purple-500/10 text-purple-300 border-purple-500/30'}`}>
          {isIp ? <><Globe className="w-3 h-3" /> IP / VPN</> : <><Monitor className="w-3 h-3" /> URZĄDZENIE</>}
        </span>
        {info?.country && <span className="text-[11px] text-slate-400 flex items-center gap-1 font-mono"><span>{getFlag(info.country, info.countryCode)}</span> {info.country}{info.city ? ` • ${info.city}` : ''}</span>}
      </div>

      <div className="font-mono text-[13px] text-orange-300 break-all bg-black/40 p-2.5 rounded-lg border border-white/5">
        {isIp ? id : `${id.slice(0, 22)}...`}
      ...
      <div className="mt-3 space-y-2 text-xs font-mono flex-1">
        {info ? (
          <>
            <div className="flex justify-between items-center bg-white/[0.03] px-2.5 py-2 rounded-md text-slate-400"><span className="flex items-center gap-1.5"><Laptop className="w-3.5 h-3" /> Przeglądarka</span><span className="text-slate-200 truncate max-w-[130px]">{info.browserFamily || info.browser || 'Chrome'} / {info.osFamily || 'Win'}</span></div>
            <div className="flex justify-between items-center bg-white/[0.03] px-2.5 py-2 rounded-md text-slate-400"><span className="flex items-center gap-1.5"><Smartphone className="w-3.5 h-3" /> Urządzenie</span><span className="text-slate-200">{info.deviceType || 'desktop'}</span></div>
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-white/[0.03] px-2.5 py-2 rounded-md"><div className="text-[10px] text-slate-500">Kąt</div><div className="text-cyan-300 font-bold">{info.angle ?? '—'}°</div></div>
              <div className="bg-white/[0.03] px-2.5 py-2 rounded-md"><div className="text-[10px] text-slate-500">Status</div><div className="text-red-400 font-bold">FAIL</div></div>
            </div>
            <div className="flex justify-between items-center px-1 pt-1 text-[11px] text-slate-400"><span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {info.createdAt ? new Date(info.createdAt).toLocaleString("pl-PL") : 'teraz'}</span><button onClick={()=>onDetails(info)} className="text-cyan-400 hover:text-cyan-300 flex items-center gap-1"><Eye className="w-3 h-3" /> Szczegóły</button></div>
          </>
        ) : (
          <div className="text-slate-500 text-center py-6">Brak danych w historii</div>
        )}
      </div>
        ...
        <Card className="bg-slate-800 border-slate-700 mb-6">
          <div className="p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 className="text-lg font-bold text-white font-mono flex items-center gap-2">Zablokowane urządzenia / IP <span className="text-xs bg-orange-500/20 text-orange-300 px-2.5 py-0.5 rounded-full border border-orange-500/20">{lockedIPs.length} blokad</span></h2>
                      ...
                      <tr key={idx} className={`border-b border-slate-700/50 hover:bg-slate-700/30 cursor-pointer ${isLocked ? "bg-red-900/20" : ""}`} onClick={()=>setSelected(a)}>
                        ...
                        <td className="py-3 px-3">
                          <div className="flex flex-col">
                            <span className="text-[11px] text-purple-300 font-mono bg-purple-500/10 border border-purple-500/20 px-2 py-0.5 rounded w-fit">{a.fingerprint ? `${a.fingerprint.slice(0,8)}...` : a.browserFamily || '—'}</span>
                            <span className="text-[10px] text-slate-500 mt-0.5">{a.osFamily || ''} {a.deviceType ? `• ${a.deviceType}` : ''}</span>
                          </div>
                        </td>
                        <td className="py-3 px-3">
                          <button onClick={(e)=>{e.stopPropagation(); setSelected(a)}} className="flex items-center gap-1.5 hover:text-cyan-300 text-slate-300 border-b border-dotted border-slate-500 hover:border-cyan-400 transition-colors text-left">
                        ...
                        <td className="py-3 px-3" onClick={e=>e.stopPropagation()}><UnlockButton ipAddress={a.fingerprint || a.ipAddress} /></td>
                      </tr>
                    );
                  })}
                </tbody>
