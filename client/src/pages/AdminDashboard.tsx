      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">Statystyki i zarz?dzanie pr車bami</p>
          </div>
                  ...
                  {attempts.map((attempt: any, idx: number) => {
                    ...
                    return (
                      <tr key={idx} className={`border-b border-slate-700/50 hover:bg-slate-700/30 ${isLocked? 'bg-red-900/20' : ''}`}>
                        <td className={`py-3 px-3 ${isLocked? 'text-red-400 font-bold' : 'text-slate-300'}`}>{attempt.ipAddress} {isLocked && <span className="text-[10px] bg-red-500/20 text-red-400 px-2 py-0.5 rounded-full">ZABLOKOWANE</span>}</td>
                        ...
                        <td className="py-3 px-3 text-slate-300 overflow-visible">
                          <div className="group relative inline-block overflow-visible">
                            <span className="flex items-center gap-1.5 cursor-help border-b border-dotted border-slate-500 hover:text-cyan-300">
                              <span>{getCountryFlag(attempt.country)}</span><span>{attempt.country}</span>{attempt.city && <span className="text-slate-400 text-xs">({attempt.city})</span>}<Info className="w-3 h-3" />
                            </span>
                            <div className="absolute left-0 top-full mt-2 hidden group-hover:block z-[9999] bg-slate-900 border-2 border-slate-600 rounded-xl p-4 text-xs text-slate-200 shadow-2xl min-w-[340px]">
                              ...
                              <div className="space-y-1.5">
                                ...
                                {attempt.regionName && <div>??? Region: {attempt.regionName}</div>}
                                ...
                                {attempt.latitude && <div>?? Wsp車?rz?dne: {attempt.latitude}, {attempt.longitude}</div>}
                              </div>
                              <div className="border-t border-slate-700 mt-3 pt-2 text-[11px] text-slate-400 space-y-1">
                                {attempt.browserFamily && <div>?? Przegl?darka: {attempt.browserFamily}</div>}
                                {attempt.osFamily && <div>?? System: {attempt.osFamily}</div>}
                                {attempt.deviceType && <div>?? Urz?dzenie: {attempt.deviceType}</div>}
                              </div>
                              {attempt.latitude && <div className="mt-3"><a href={`https://www.openstreetmap.org/?mlat=${attempt.latitude}&mlon=${attempt.longitude}&zoom=14`} target="_blank" className="bg-cyan-600 hover:bg-cyan-500 text-white px-3 py-1.5 rounded-md text-[11px] font-bold">??? Zobacz na mapie</a></div>}
                            </div>
                          </div>
                        </td>
