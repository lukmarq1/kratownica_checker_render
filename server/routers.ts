  ...
  angle: router({
    ...
    verify: publicProcedure.input(z.object({angle:z.number(),fingerprint:z.string().optional(),deviceId:z.string().optional(),browser:z.string().optional(),os:z.string().optional()})).mutation(async ({ctx,input})=>{
      ...
      if (isInstantVpnBan) {
        ...
        return { success: false, reason: "vpn_detected", isVpn: true, isLocked: true, locked: true, remainingAttempts: 0, remaining: 0, remainingLockoutMs: LOCKOUT_MS, blockedBy: ip, message: `VPN/Proxy/Hosting wykryty (${geo?.isp||geo?.org}) - blokada 24h` };
      }
      const baseKeys=[fingerprint,deviceId,ip,subnet,geoKey].filter(Boolean) as string[];
      const linkedKeys=getRecentLinkedKeys(fingerprint, deviceId);
      const keysToCheck=Array.from(new Set([...baseKeys,...linkedKeys]));
      ...
      if(isCorrect){ await resetAttempts(keysToCheck,ip,fingerprint); return {success:true,reason:"correct",angle:input.angle}; }
        ...
        if(locked.length>0){
          ...
          return {success:false,reason:"locked",remainingAttempts:0,remaining:0,isLocked:true,locked:true,lockedUntil:first.r.lockedUntil,remainingLockoutMs:first.r.lockedUntil?first.r.lockedUntil.getTime()-Date.now():LOCKOUT_MS,isVpn,blockedBy:first.key,allBlockedBy:locked.map(x=>x.key)};
        }
        const min=Math.min(...all.map(x=>x.r.remainingAttempts));
        return {success:false,reason:isVpn?"vpn_detected":"incorrect",remainingAttempts:min,remaining:min,isLocked:false,locked:false,isVpn,blockedBy:undefined};
      }
    }),
  }),
  admin: router({
