...
// TWARDA BLOKADA ANTY-OSZUST - całe WiFi po 2 failach
const MAX_ATTEMPTS = 2; // per urządzenie
const MAX_SUBNET_ATTEMPTS = 3; // per /24
const MAX_GEO_ATTEMPTS = 4; // per miasto+ISP - łapie 3 telefony + 2 laptopy na tym samym WiFi
...
async function resetAttempts(keys: string[], ip: string, fingerprint?: string) {
  for (const key of keys) {
    if (key.startsWith("geo:")) continue; // geo nie resetujemy - chroni przed oszustem z wieloma urządzeniami
    const rec = getOrCreateRecord(key);
...
function detectVpnUsage(fingerprint: string, currentIp: string, currentGeo: any): boolean {
  if (!fingerprint) return !!(currentGeo?.isHosting || currentGeo?.isProxy);
  const recent = historyStore.filter(h => h.fingerprint === fingerprint && Date.now() - h.createdAt.getTime() < VPN_DETECTION_WINDOW_MS);
  if (recent.length === 0) return !!(currentGeo?.isHosting || currentGeo?.isProxy);
...
export const appRouter = router({
  ...
  angle: router({
    getStatus: publicProcedure.input(z.object({ fingerprint: z.string().optional(), deviceId: z.string().optional() })).query(async ({ ctx, input }) => {
      ...
      // TWARDA BLOKADA: sprawdzamy WSZYSTKO - urządzenie + IP + /24 + geo
      const keysToCheck = Array.from(new Set([fingerprint, deviceId, ip, subnet, geoKey].filter(Boolean) as string[]));
      for (const k of keysToCheck) { if (await isLocked(k)) { return { isLocked: true, locked: true, remainingLockoutMs: await getRemainingLockoutTime(k), remainingMs: await getRemainingLockoutTime(k), blockedBy: k }; } }
      const primaryKey = fingerprint || deviceId || ip;
      const rec = primaryKey ? attemptStore.get(primaryKey) : undefined;
      const failed = rec?.failedAttempts || 0;
      const attemptsLeft = Math.max(0, MAX_ATTEMPTS - failed);
      return { isLocked: false, locked: false, failedAttempts: failed, remainingAttempts: attemptsLeft, remainingLockoutMs: 0, remainingMs: 0, attemptsLeft, maxAttempts: MAX_ATTEMPTS };
    }),
    verify: publicProcedure.input(z.object({ angle: z.number(), fingerprint: z.string().optional(), deviceId: z.string().optional(), browser: z.string().optional(), os: z.string().optional() })).mutation(async ({ ctx, input }) => {
      ...
      // TWARDA BLOKADA: to samo WiFi = ta sama sieć = oszust zablokowany od razu
      const keysToCheck = Array.from(new Set([fingerprint, deviceId, ip, subnet, geoKey].filter(Boolean) as string[]));
      ...
      if (isCorrect) { await resetAttempts(keysToCheck, ip, fingerprint); return { success: true, reason: "correct", angle: input.angle }; }
        ...
        if (locked.length > 0) {
          ...
          return { success: false, reason: "locked", remainingAttempts: 0, remaining: 0, isLocked: true, locked: true, lockedUntil: firstLocked.r.lockedUntil, remainingLockoutMs: firstLocked.r.lockedUntil ? firstLocked.r.lockedUntil.getTime() - Date.now() : LOCKOUT_MS, isVpn, blockedBy: firstLocked.key, allBlockedBy: locked.map(x=>x.key) };
        }
  ...
  admin: router({
    ...
    unlockIp: publicProcedure.input(z.object({ ipAddress: z.string().optional(), fingerprint: z.string().optional(), deviceId: z.string().optional(), subnet: z.string().optional(), geoKey: z.string().optional() })).mutation(async ({ input }) => {
      ...
      if (input.ipAddress) { const s = getSubnet(input.ipAddress); if (s) keysToDelete.add(s); }
      for (const h of historyStore) {
        ...
        if (matches) {
          if (h.ipAddress) { keysToDelete.add(h.ipAddress); const s = getSubnet(h.ipAddress); if (s) keysToDelete.add(s); }
          ...
          const gk = getGeoKeyFromHistory(h); if (gk) keysToDelete.add(gk);
        }
      }
      for (const [storeKey, rec] of attemptStore.entries()) {
        ...
        if (shouldDelete) { keysToDelete.add(storeKey); rec.ips.forEach(ip => keysToDelete.add(ip)); rec.fingerprints.forEach(fp => { if (fp !== "unknown") keysToDelete.add(fp); }); }
      }
      for (const k of Array.from(keysToDelete)) { const rec = attemptStore.get(k); if (rec) { rec.ips.forEach(ip => keysToDelete.add(ip)); rec.fingerprints.forEach(fp => { if (fp !== "unknown") keysToDelete.add(fp); }); } }
      let deletedCount = 0; for (const k of keysToDelete) { if (attemptStore.delete(k)) deletedCount++; }
