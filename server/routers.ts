...
async function incrementFail(keys:string[],fp?:string,ip?:string,extra?:{deviceId?:string,browser?:string,os?:string}){ if(!keys.length) return {locked:false,duration:BASE_LOCKOUT_MS}; await ensureTable(); const recs=await Promise.all(keys.map(k=>getRecord(k).catch(()=>null))); const shouldLock=recs.some(r=>((r?.failed_attempts||0)+1)>=MAX_ATTEMPTS); if(shouldLock){ const d=await lockKeys(keys,fp,ip); const ipKey=ip||"unknown"; const payload={ip:ipKey, subnet: (ipKey&&ipKey!=="unknown")? (getSubnet(ipKey)+".0/24"): "unknown", fingerprint:fp||keys[0]||"", deviceId:extra?.deviceId||"", browser:extra?.browser||"", os:extra?.os||"", reason: d>BASE_LOCKOUT_MS?'REPEAT_LOCKOUT':'MAX_ATTEMPTS', count: MAX_ATTEMPTS, geo:null}; console.log('[Lock] shouldLock=true, wysyłam alerty dla', ipKey, payload.reason); sendTelegramBlock(payload).then(ok=>console.log('[Telegram] wynik alertu blokady:', ok)).catch(e=>console.error('[Telegram] nieoczekiwany błąd promise', e)); return {locked:true,duration:d}; } const p=getPool(); await Promise.all(keys.map(async k=>{ const r=recs[keys.indexOf(k)]; const fails=(r?.failed_attempts||0)+1; await p.query(`INSERT INTO lockouts (lock_key,failed_attempts,last_attempt_at) VALUES (?,?,NOW()) ON DUPLICATE KEY UPDATE failed_attempts=?, last_attempt_at=NOW()`,[k,fails,fails]).catch(()=>{}); })); return {locked:false,duration:BASE_LOCKOUT_MS}; }
